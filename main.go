...
var itf = flag.String("ip", "", "-ip: ip 地址")
var debug = flag.Bool("debug", false, "-debug:调试数据打印")
...
func readARP(handle *pcap.Handle, iface *net.Interface, stop chan struct{}) {
	...
	for {
		...
		case packet = <-in:
			...
			if net.IP(arp.SourceProtAddress).String() == *itf {
				if *debug {
					log.Printf("IP %v is at %v", net.IP(arp.SourceProtAddress), net.HardwareAddr(arp.SourceHwAddress))
				}
				fmt.Println(strings.ToUpper(strings.ReplaceAll(net.HardwareAddr(arp.SourceHwAddress).String(), ":", "")))
			}
		}
	}
}
